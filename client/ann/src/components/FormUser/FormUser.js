...
const FormUser = ({ user, operation }) => {
  ...
  const call_alert = (text) => {
    ...
    setTimeout(() => alert.classList.remove("alert_active"), 1000);
  };
  ...
  return (
    <div>
      <form
        onSubmit={
          operation == "registration" ? onSubmitFormReg : onSubmitFormEdit
        }
      >
        <div className="img_container">
          <label for="backimg_file">
            {backimg[0] == null ? (
              <div className="  form_backimg"></div>
            ) : (
              <img
                className="back_img"
                id="backimg_preview"
                src={`http://localhost:4000/${backimg[0]}`}
              />
            )}
            <p className="img_description img_back">
              <span>
                <i class="fa fa-camera" aria-hidden="true"></i>choose back
              </span>
            </p>
          ...
          <label className="avatar_div" for="avatar_file">
            <img
              ...
              src={
                avatar[0]
                  ? `http://localhost:4000/${avatar[0]}`
                  : "http://localhost:4000/static/default-avatar.png"
              }
            />
            <p className="img_description img_avatar_description">
              <span>
                <i class="fa fa-camera" aria-hidden="true"></i>choose avatar
              </span>
            </p>
            <input
              type="file"
        ...
        {operation == "registration" ? (
          ...
          <div className="profile_btn_container">
            <button
              className="submit-button model_cancel"
              onClick={(e) => {
                e.preventDefault();
                const model = document.querySelector(".profile_model");
                console.log(model);
            ...
            <button className="submit-button profile_edit_btn" type="submit">
              edit
            </button>
          </div>
        )}
