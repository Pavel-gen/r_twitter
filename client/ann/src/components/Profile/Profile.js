        ...
        return (
            ...
            <div className='section_p'>
                <div className='container_p'>
                    {user.backimg == undefined ? <div className='back_img  red_back' ></div> :
                           <img className='back_img' src={`http://localhost:4000/${user.backimg}`} />     
                }
                    <div className=''>
                    
                        <div className='f_level' >
                            <img  id='profile_photo' src={`http://localhost:4000/${user.avatar}`} />
                            <div className='profile_button_container'>
                            {user._id !== user_id ? (
                                <>
                                    <button className='profile_mess  ' ><i className="fa fa-envelope" aria-hidden="true"></i></button>
                                    <button   className={condition ? 'edit_profile_btn follow_btn': 'edit_profile_btn'} onClick={(e) => {
                                        
                                        const el = e.currentTarget
                                        el.classList.toggle('follow_btn')
                                        e.preventDefault()
                                        Follow(user._id)
                                    }} >{condition ? 'folowing': 'follow'}</button>
                                </>
                            )
                            :
                             <button className='edit_profile_btn' onClick={(e) => showProfileModel()} >edit</button> 
                             }
                            </div>
                        </div>
