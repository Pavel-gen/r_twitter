...
import './Tweet.css'
import postSlice, { update, delete_post, update_post, changed_content, changed_id } from '../../fearutures/postSlice'
...
const Tweet = ({author, content, date, id, likes, likedBy} ) => {
    ...
    <div id={id} className='container_t' >  
      <img className='img_post' src={`http://localhost:4000/${author.avatar}`} />    
        ...
        <div  className='start_cont'>
            <button onClick={(e) => showMenu(e.currentTarget)} className='btn_start '><i className='fa fa-cog'></i></button>
            <div className='btn_container' >
                {author._id == user_id ? (
                <>
                 <button onClick={(e) => removePost(id, dispatch, e.currentTarget.parentElement)} className='btn btn_delete' >delete</button>
                 <button className='btn btn_update' onClick={(e) => {
            ...
            >update</button> 
                 ...
                 (
                     <>
                            <button className='btn btn_delete' onClick={(e) => window.location.assign(`http://localhost:3000/profile/${author._id}`)} >перейте к профилю</button>
                            <button className='btn btn_delete' onClick={(e) => showMenu(e.currentTarget.parentElement)} >отмена</button>                     
                     </>
        ...
        <div className='foot_t' >
            <span><i className={likeCondition ? 'fa fa-heart like pressed_like' : 'fa fa-heart like' } aria-hidden="true" onClick={(e)=> {
                e.currentTarget.classList.toggle('pressed_like')
                Like()
            }}></i>{altLikes !== 0 && altLikes}</span>
            <span><i className="fa fa-retweet retweet" aria-hidden="true"></i>2</span>
            <span><i className="fa fa-comment " aria-hidden="true"></i>12</span>
        </div>
        </div>
