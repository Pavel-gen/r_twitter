...
router.post('/posts',check('content', 'Content не может быть пустым').notEmpty(), authMiddleware ,TweetController.create)
...
router.post('/users/registration', uploadingImg,
check('username', 'Имя полтьзователя не может быть пустым').notEmpty(),
check('password', 'Пороль должен быть длиннее 4').isLength({min: 4}), UserController.registration)
